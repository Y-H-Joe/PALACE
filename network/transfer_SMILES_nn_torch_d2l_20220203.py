...
# y_hat的每个元素都是值(值是50个y train)的加权平均值，其中的权重是注意力权重
y_hat = torch.matmul(attention_weights, y_train)
plot_kernel_reg(y_hat)

d2l.show_heatmaps(attention_weights.unsqueeze(0).unsqueeze(0), ## 两个unsqueeze让2D的matrix变成4D,符合show_heatmaps的要求
...
# X_tile[(1 - torch.eye(n_train)).type(torch.bool)].shape = 2450
# X_tile[(1 - torch.eye(n_train)).type(torch.bool)].reshape((n_train, -1)).shape = [50,49]
...
attention = AdditiveAttention(key_size=2, query_size=20, num_hiddens=8,
                              dropout=0.1)
## 使用PyTorch进行训练和测试时一定注意要把实例化的model指定train/eval。model.eval()时，框架会自动把BN和Dropout固定住，不会取平均，而是用训练好的值
attention.eval()
