...
from modules_v3 import (printer,PALACE_SMILES,logging,predict_PALACE_SMILES,model_diagnose,
                    PALACE_Encoder_v2,assign_gpu,PALACE_Decoder_v2,retrieve_vocab)


def main(model_dp, data_dp, smi_vocab_dp, output_dp, beam = 5):
# ===============================Settings======================================
#%% Settings
    class Args:
        def __init__(self):
...
#%% Predicting
    ...
    with open(data_dp,'r') as r:
        [samples.append(x.strip().split('\t')[3]) for x in r.readlines()]
    ...
    with open(output_dp,'w') as o:
        for src in samples:
            prediction = predict_PALACE_SMILES(net, src, smi_vocab, args.num_steps,device,beam,save_attention_weights=False)
...
if __name__ == '__main__':
    ...
    output_dp = 'aa'
    """
    main(model_dp, data_dp, smi_vocab_dp, output_dp)
