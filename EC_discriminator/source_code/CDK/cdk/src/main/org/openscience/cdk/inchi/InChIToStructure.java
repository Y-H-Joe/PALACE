...
import org.openscience.cdk.tools.SingleElectronAdder;
...
protected JniInchiInputInchi input;
    ...
    protected void generateAtomContainerFromInchi(IChemObjectBuilder builder) throws CDKException {
        ...
        //add single electrons by iterating over each atom:
        SingleElectronAdder adder = SingleElectronAdder.getInstance();
        for(IAtom atom : molecule.atoms()){
 			if(atom.getFormalCharge() == 0){//only for uncharged atoms for now
				adder.perceiveSingleElectrons(molecule, atom);
 			}
 		}
    ...
    private void addDeuteriums(IChemObjectBuilder builder, JniInchiAtom iAt,
    		IAtom cAt) {
    	//deuterium:
    	int numD = iAt.getImplicitDeuterium();
    	XMLIsotopeFactory isof = null;
    	try {
    		isof = XMLIsotopeFactory.getInstance(builder);
    	} catch (IOException e1) {}

    	for(int iso = 0; iso < numD; iso++){
    		IElement e = new Isotope("H", 2);
    		IAtom D = new Atom(e);
    		isof.configure(D);
    		D.setFormalCharge(0);
    		molecule.addAtom(D);
