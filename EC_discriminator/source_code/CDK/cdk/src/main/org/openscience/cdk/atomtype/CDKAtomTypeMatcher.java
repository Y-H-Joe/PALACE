...
public class CDKAtomTypeMatcher implements IAtomTypeMatcher {
    ...
    private IAtomType perceiveCarbonRadicals(IAtomContainer atomContainer, IAtom atom) throws CDKException {
    	...
    	if(atomContainer.getConnectedSingleElectronsCount(atom) == 1){
    		if (atomContainer.getConnectedBondsCount(atom) == 0) {
    			...
    			if (maxBondOrder == IBond.Order.SINGLE) {
    				...
    				if (atomContainer.getConnectedSingleElectronsCount(atom) != 2) {
    	...
    	} else if(atomContainer.getConnectedSingleElectronsCount(atom) == 2){
    		if (atomContainer.getConnectedBondsCount(atom) == 0) {
    			IAtomType type = getAtomType("C.biradical.sp3");
    			if (isAcceptable(atom, atomContainer, type)) return type;
    		} else if (atomContainer.getConnectedBondsCount(atom) <= 3) {
    			IBond.Order maxBondOrder = atomContainer.getMaximumBondOrder(atom);
    			if (maxBondOrder == IBond.Order.SINGLE) {
    				...
    				if (atomContainer.getConnectedSingleElectronsCount(atom) != 2) {
    					IAtomType type = getAtomType("C.biradical.sp2");
    					if (isAcceptable(atom, atomContainer, type)) return type; 
    				}
    			}
    		}
    	} else if(atomContainer.getConnectedSingleElectronsCount(atom) == 3){
    		IAtomType type = getAtomType("C.triradical.sp3");
    		if (isAcceptable(atom, atomContainer, type)) return type;
    	}
        return null;
    }
    ...
    private IAtomType perceiveOxygenRadicals(IAtomContainer atomContainer, IAtom atom) throws CDKException {
    	...
    	if (atom.getFormalCharge() == 0) {
    		if (atomContainer.getConnectedBondsCount(atom) == 0) {    			
    			if(atomContainer.getSingleElectronCount() == 1){
    				IAtomType type = getAtomType("O.sp3.radical");
    				if (isAcceptable(atom, atomContainer, type)) return type;
    			}
    			else if(atomContainer.getSingleElectronCount() == 2){
    				IAtomType type = getAtomType("O.sp3.atom");
    				if (isAcceptable(atom, atomContainer, type)) return type;
    			}
            }
    ...
    throws CDKException {
        ...
        if (hasOneSingleElectron(atomContainer, atom)) {
        	return perceiveSulfurRadicals(atomContainer, atom);
    ...
    private IAtomType perceiveSulfurRadicals(IAtomContainer atomContainer,
			IAtom atom) throws CDKException{
        
    	if (atomContainer.getConnectedBondsCount(atom) == 1) {
    		IAtomType type = getAtomType("S.sp3.radical");
    		if (isAcceptable(atom, atomContainer, type)) return type;
    	}
    ...
    throws CDKException {
        ...
        if (countSingleElectrons(atomContainer, atom) == 3) {
            ...
            if(atom.getFormalCharge() == null ||
            	...
            	if(neighborcount == 2){
            		IAtomType type = getAtomType("P.sp2.radical");
                    if (isAcceptable(atom, atomContainer, type)) return type;
            	}
            	else if(neighborcount == 3){
            		if (maxBondOrder == CDKConstants.BONDORDER_DOUBLE) {
                		IAtomType type = getAtomType("P.sp3d1.radical");
                        if (isAcceptable(atom, atomContainer, type)) return type;
            		}
            	}
            }
            ...
            if (atom.getFormalCharge() != null &&
                ...
                if (isAcceptable(atom, atomContainer, type)) return type;
                	...
                	IAtomType type = getAtomType("P.sp3d1");
                    if (isAcceptable(atom, atomContainer, type)) return type;
            } else {
                IAtomType type = getAtomType("P.ine");
                if (isAcceptable(atom, atomContainer, type)) return type;
            ...
            // count the number of double bonded oxygens
            int doubleBonds = countAttachedDoubleBonds(atomContainer, atom);
            if (atom.getFormalCharge() != null && atom.getFormalCharge() == 1 && doubleBonds == 0) {
    ...
    private IAtomType perceiveSilicon(IAtomContainer atomContainer, IAtom atom) throws CDKException {
        if (hasOneSingleElectron(atomContainer, atom)) {
            ...
            if (atomContainer.getConnectedBondsCount(atom) == 2) {
                ...
                if (isAcceptable(atom, atomContainer, type)) return type;
            } else if (atomContainer.getConnectedBondsCount(atom)+atom.getImplicitHydrogenCount() == 4) {
