...
public class Reaction extends ChemObject implements Serializable, IReaction, Cloneable {
	...
	public Object clone() throws CloneNotSupportedException {
        ...
        for (int i = 0; i < products.getAtomContainerCount(); ++i){
        	...
        	IAtomContainer mol2 = clone.products.getAtomContainer(i);
        	for (int j = 0; j < mol.getAtomCount(); ++j) atomatom.put(mol.getAtom(j), mol2.getAtom(j));
        }
	...
	/**
     * Check whether the reaction is balanced. Uses atomic numbers to assess.
     * 
     * @return is reaction balanced
     * @throws NoSuchAtomTypeException 
     */
	public boolean checkAtomBalance() throws NoSuchAtomTypeException{
		int [] elements	=	new int[118];
		
		for(int i = 0;	i < this.reactants.getAtomContainerCount();	i++){
			IAtomContainer react	=	this.reactants.getAtomContainer(i);
			for(int atcount = 0;	atcount < react.getAtomCount();	atcount++){
				if(react.getAtom(atcount).getSymbol().equals("X1000")){}
				else{
					int atomicNumber	=	react.getAtom(atcount).getAtomicNumber();
				
					if(atomicNumber > 118){
						throw new NoSuchAtomTypeException("Unknown atom type detected in reaction");
					}
		...
		for(int i = 0;	i < this.products.getAtomContainerCount();	i++){
			IAtomContainer prod		=	this.products.getAtomContainer(i);
			for(int atcount = 0;	atcount < prod.getAtomCount();	atcount++){
				if(prod.getAtom(atcount).getSymbol().equals("X1000")){}
				else{				
