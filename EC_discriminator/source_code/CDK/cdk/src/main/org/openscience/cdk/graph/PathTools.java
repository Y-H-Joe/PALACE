    ...
    public static boolean depthFirstTargetSearch(IAtomContainer molecule, IAtom root, IAtom target, IAtomContainer path) {
        ...
        for (IBond bond : bonds) {
            ...
            if (!nextAtom.getFlag(CDKConstants.VISITED)) {
                ...
                path.addBond(bond);
                if (nextAtom == target) {
                	//if a path is found, add the root atom.
                	path.addAtom(root);
                    return true;
                } else {
        ...
        for (IAtom atom : sphere) {
            ...
            for(IStereoElement el : atomContainer.stereoElements()){
            	if(el instanceof ITetrahedralChirality){
            		ITetrahedralChirality ap = (ITetrahedralChirality)el;
            		if(ap.getChiralAtom() == atom){
            			molecule.addStereoElement(el);
            		}
            	}
            }
            // now look at bonds
...
//					logger.debug("wie oft???");
    ...
    /**Does the same as breadthFirstTargetSearch without molecule in arguments, only here all visited atoms and
     * bonds are added to a molecule.
     * Bonds are only added if neither of the connected atoms have been visited.
     * 
     * @param atomContainer
     * @param sphere
     * @param target
     ...
     * @return
     */
    public static int breadthFirstTargetSearch(IAtomContainer atomContainer, List<IAtom> sphere, IAtom target, IAtomContainer molecule, int pathLength, int cutOff){
    	...
    	if (pathLength == 0) resetFlags(atomContainer);
         ...
         for (IAtom atom : sphere) {
        	 ...
        	 for (IBond bond : bonds) {
        		 if (!bond.getFlag(CDKConstants.VISITED)) {
        			 bond.setFlag(CDKConstants.VISITED, true);
        			 //Only add bond if other end of the bond has not been connected: avoids closing cycles and
        			 //bypassing shortest path.
        			 if(!bond.getConnectedAtom(atom).getFlag(CDKConstants.VISITED)){
        				 molecule.addBond(bond);
        				 for(IStereoElement el : atomContainer.stereoElements()){
        					 if(el instanceof IDoubleBondStereochemistry){
        						 IDoubleBondStereochemistry db = (IDoubleBondStereochemistry) el;
        						 if(db.getStereoBond() == bond)
        							 molecule.addStereoElement(el);
        					 }
        				 }
        			 }
        		 ...
        		 if (!nextAtom.getFlag(CDKConstants.VISITED)) {
        			 if (nextAtom == target) {
        				 ...
        				 List<ILonePair> nextLonePairs = atomContainer.getConnectedLonePairsList(nextAtom);
        				 for (ILonePair lonePair : nextLonePairs) molecule.addLonePair(lonePair);
        				 List<ISingleElectron> nextSingleElectrons = atomContainer.getConnectedSingleElectronsList(nextAtom);
        				 for (ISingleElectron singleElectron : nextSingleElectrons) molecule.addSingleElectron(singleElectron);
        				 for(IStereoElement el : atomContainer.stereoElements()){
        					 if(el instanceof ITetrahedralChirality){
        						 ITetrahedralChirality ap = (ITetrahedralChirality)el;
        						 if(ap.getChiralAtom() == atom){
         ...
         if (newSphere.size() > 0) {
        	 return breadthFirstTargetSearch(atomContainer, newSphere, target, molecule, pathLength, cutOff);
         }
         return -1;
    }
