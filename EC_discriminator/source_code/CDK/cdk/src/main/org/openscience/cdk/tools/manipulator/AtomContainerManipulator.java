...
public class AtomContainerManipulator {
    ...
    public static void convertImplicitToExplicitHydrogens(IAtomContainer atomContainer) {
        ...
        for (IAtom atom : atomContainer.atoms()) {
        	...
        	if(atom.getImplicitHydrogenCount() != null && atom.getImplicitHydrogenCount() != 0){
        		//Original if condition: don't do it for hydrogens... 
        		//Better if don't do it if hydrogen count isn't set or 0
    ...
    public static IAtomContainer removeHydrogens(IAtomContainer atomContainer)
    {
                ...
                i++)
        {
            ...
            if (!atom.getSymbol().equals("H"))
            {
                ...
                //copy single electrons:
				for(ISingleElectron se : atomContainer.singleElectrons()){
					if(atomContainer.getAtomNumber(se.getAtom()) == atomContainer.getAtomNumber(atom)){
						int atomID = mol.getAtomNumber(clonedAtom);
						mol.addSingleElectron(atomID);
					}
				}
            }
            else
        ...
        //clone tetrahedral stereoelements:
        for(IStereoElement el : atomContainer.stereoElements()){
        	if(el instanceof ITetrahedralChirality){
        		ITetrahedralChirality tc = (ITetrahedralChirality)el;
        		List<IAtom>ligands = new ArrayList<IAtom>();
        		for(IAtom a : tc.getLigands()){
        			if(a.getSymbol().equals("H")) ligands.add(map.get(tc.getChiralAtom()));
        			else ligands.add(map.get(a));
        		}
        		mol.addStereoElement(new TetrahedralChirality(map.get(tc.getChiralAtom()),
        				ligands.toArray(new IAtom[ligands.size()]), tc.getStereo()));
        	}
        }
        mol.setProperties(atomContainer.getProperties());
        mol.setFlags(atomContainer.getFlags());
    ...
    public static void clearAtomConfigurations(IAtomContainer container) {
    	//If there is only one atom in the container, it will not be possible to find out whether the remaining atom
    	//is ion, radical or atom!
    	if(container.getAtomCount()==1){
    		...
    		for (IAtom atom : container.atoms()) {
    			...
    			//Do not unset formal charges. The atom typer perciever uses these charges to correctly percieve 
    			//atoms. If unset ions will never be correctly detected.
    			...
    			//Be careful with the unsetting of the Atomic number. It makes no sense to unset the atomic number 
    			//but not the symbol of the atom...
     ...
     *
     * @param container {@link IAtomContainer} to search the {@link ITetrahedralChirality} for.
     * @param atom      {@link IAtom} for which the {@link ITetrahedralChirality} must be defined.
     * @return          the {@link ITetrahedralChirality} or null if none is define for the given atom.
     */
    public static ITetrahedralChirality getAtomParity(IAtomContainer container, IAtom atom) {
        for (IStereoElement element : container.stereoElements()) {
            if (element instanceof ITetrahedralChirality) {
            	ITetrahedralChirality parity = (ITetrahedralChirality)element;
                if (parity.getChiralAtom() == atom) return parity;
            }
        }
        return null;
    }
    
    public static void fixImplicitHydrogenCount(IAtomContainer container){
    	for(IAtom atom:container.atoms()){
    		if(atom.getImplicitHydrogenCount() == null){
    			atom.setImplicitHydrogenCount(atom.getFormalNeighbourCount()
	...
	public static void fixAromaticity(IAtomContainer ac) throws CDKException{
		AtomContainerManipulator.percieveAtomTypesAndConfigureUnsetProperties(ac);
		AtomContainerManipulator.percieveAtomTypesAndConfigureAtoms(ac);
		//daylight aromaticity model to allow for correct smarts matching.
		ElectronDonation ed=ElectronDonation.daylight();
		CycleFinder cf=Cycles.allOrVertexShort();
		Aromaticity arom=new Aromaticity(ed,cf);
		arom.apply(ac);
	}
}
