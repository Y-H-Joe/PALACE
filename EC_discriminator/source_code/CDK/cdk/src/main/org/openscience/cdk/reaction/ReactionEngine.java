...
public class ReactionEngine{
	...
	public ReactionEngine(){
		//Do not read dictionary every time an engine is generated, its always the same anyways. Generate once.
		if(dictionary == null){
			DictionaryDatabase db = new DictionaryDatabase();
			dictionary = db.getDictionary("reaction-processes");
		}
		try {
			IReactionProcess reaction = (IReactionProcess) this;
	    	EntryReact entry = initiateDictionary((IReactionProcess)reaction);
	 ...
	 */
	private EntryReact initiateDictionary(IReactionProcess reaction) {
