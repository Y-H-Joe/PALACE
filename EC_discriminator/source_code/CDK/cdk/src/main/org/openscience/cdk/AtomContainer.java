...
//	}
	...
	/** 
	 *  Returns the number of carbon Atoms in this Container
	 * 
	 * @return	 The number of carbon Atoms in this Container
	 */
	public int getCAtomCount(){
		int cCount=0;
		//Use this.atoms() instead of this.atoms: after copying a molecule, a null atom is inexplicably added
		for(IAtom atom:this.atoms()){
	...
	/** 
	 *  Returns the number of oxygen Atoms in this Container
	 * 
	 * @return	 The number of oxygen Atoms in this Container
	 */
	public int getOAtomCount(){
		int oCount=0;
		//Use this.atoms() instead of this.atoms: after copying a molecule, a null atom is inexplicably added
		for(IAtom atom:this.atoms()){
	...
	/**
	 * 	Returns the number of nitrogen Atoms in this Container
	 * 
	 * @return	The number of nitrogen Atoms in this Container
	 */
	public int getNAtomCount(){
		int nCount=0;
		//Use this.atoms() instead of this.atoms: after copying a molecule, a null atom is inexplicably added
		for(IAtom atom:this.atoms()){
			if(atom.getAtomicNumber()==5){
				nCount++;
			}
		}
		return nCount;
	...
	/**
	 * 	Returns the number of sulphur Atoms in this Container
	 * 
	 * @return	The number of sulphur Atoms in this Container
	 */
	public int getSAtomCount(){
		int sCount=0;
		//Use this.atoms() instead of this.atoms: after copying a molecule, a null atom is inexplicably added
		for(IAtom atom:this.atoms()){
	...
	/**
	 *  Returns the number of double Bonds in this Container.
	 *
	 *@return    The number of double Bonds in this Container
	 */
	public int getDoubleBondCount(boolean includeAromatic){
		int count = 0;
		for(IBond bond:this.bonds){
			if(bond != null){
				boolean arom	=	false;
				if(!includeAromatic){
					arom	=	bond.getFlag(CDKConstants.ISAROMATIC);
				}
				
				if(bond.getOrder() == Order.DOUBLE && !arom ){
					count++;
				}
			}
		}
...
//	}
	...
	public int getStereoCenterCount() {
		int count=0;
		for(@SuppressWarnings("unused") IStereoElement stereoEl:stereoElements){
			count++;
		}
		return count;
	}
